...
Problem Link: https://leetcode.com/problems/check-ascii-palindromic/submissions/2117011479/  
...
<p>Construct a <span data-keyword="binary-string" class=" cursor-pointer relative text-dark-blue-s text-sm"></span> by replacing each character in <code>s</code> with the 8-bit binary representation of its ASCII value, <strong>including leading zeros</strong>, while preserving the original order of the characters.</p>

<p>Return <code>true</code> if the resulting binary string is a <span data-keyword="palindrome-string" class=" cursor-pointer relative text-dark-blue-s text-sm"></span>. Otherwise, return <code>false</code>.</p>
